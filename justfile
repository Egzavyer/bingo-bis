...
build OS="linux" ARCH="amd64":
	mkdir -p ./build/bingo
	{{ if OS == "darwin" { "env CGO_ENABLED=1 GOOS=" + OS + " GOARCH=" + ARCH + " go build -tags bingonative -o ./build/bingo/bingo_" + OS + "_" + ARCH + " ./cmd/bingo && codesign --sign - --entitlements ./build/entitlements.plist --force ./build/bingo/bingo_" + OS + "_" + ARCH } else { "env GOOS=" + OS + " GOARCH=" + ARCH + " go build -o ./build/bingo/bingo_" + OS + "_" + ARCH + (if OS == "windows" { ".exe" } else { "" }) + " ./cmd/bingo" } }}
...
server *ARGS:
	...
	{{ if os() == "macos" { "codesign --sign - --entitlements ./build/entitlements.plist --force ./build/bingo/bingo_server" } else { "" } }}
